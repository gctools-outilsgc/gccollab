...
if (!isset($_GET["guid"]) || !isset($_GET["badge_guid"])) {
...
$guid = (int) $_GET["guid"];
$badge_guid = (int) $_GET["badge_guid"];

// If is the same ETag, content didn"t changed.
$etag = md5($icontime . $size . $badge_guid . $guid);
...
if (isset($data_root)) {
	...
	$locator = new \Elgg\EntityDirLocator($badge_guid);
	$entity_path = $data_root . $locator->getPath();
	
	$filename = $entity_path . "badge/{$badge_guid}{$size}.jpg";
	$filecontents = @file_get_contents($filename);

	// try fallback size
	if (!$filecontents && $size !== "medium") {
		$filename = $entity_path . "badge/{$badge_guid}medium.jpg";
		$filecontents = @file_get_contents($filename);
	}
